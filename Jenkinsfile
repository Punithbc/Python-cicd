        ...
        steps {
            withCredentials([string(credentialsId: 'git_cred', variable: 'GITHUB_TOKEN')]) {
                sh '''
                    ...
                    cat Python-cicd/kube_repo/deployment.yml
                    sed -i "s/replaceImageTag/${BUILD_NUMBER}/g" Python-cicd/kube_repo/deployment.yml
                    git add Python-cicd/kube_repo/deployment.yml
